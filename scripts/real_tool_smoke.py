...
def main():
    root = Path(os.environ.get("ME_PROJECT_ROOT", Path(__file__).resolve().parents[1]))
    ...
    try:
        ...
        if not args.maradoner_only:
            ...
            sites = scan(regions, out / "tiny.fa", out / "tiny.meme", str(installed / ".runtime/envs/scan/bin/fimo"), out / "scan", .01)
            assert len(sites) > 0
            status["FIMO"] = {"status": "passed", "sites": len(sites)}
            run([str(installed / ".runtime/envs/r/bin/Rscript"), "-e", "library(Seurat); library(Signac); library(Matrix); x <- Matrix(matrix(c(1,0,2,3),2),sparse=TRUE); stopifnot(sum(x)==6); sessionInfo()"], out / "R.log")
        ...
        if args.with_sce2g:
            ...
            scanbin = installed / ".runtime/envs/scan/bin"
            subset_fragments(source, metadata, frag, bgzip=str(scanbin / "bgzip"), tabix=str(scanbin / "tabix"))
            pred = run_sce2g(repo, frag, out / "sce2g", str(installed / ".runtime/envs/sce2g/bin/snakemake"))
